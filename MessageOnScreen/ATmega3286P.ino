// Reads the incoming string from the Arduino serial port (that was sent from ESP8266 web server) to show it on dot matrix array.
// MD_MAX72xx used to display text on the dot matrix array.

// AI Description:
// The selected code is part of a program written in C++. It is part of a larger program that controls an LED matrix display.
// The specific part of the code that I have selected is related to the graphics routines that are used to display different animations on the LED matrix.
// The code includes several functions that are used to display different types of animations,
// such as midline, scanner, random, spectrum, heartbeat, fade, hearts, eyes, bounce ball, arrow scroll, and invader.
// Each function has its own set of parameters and variables that control how the animation is displayed.
// For example, the graphicMidline1 function is used to display a midline animation on the LED matrix.
// The function sets the intensity of the LEDs in each row of the matrix to either full brightness or off, depending on whether the function is initializing or not.
// The function then loops through each column of the matrix, setting the intensity of the LEDs in each column to either full brightness or off.
// Similarly, the graphicHeartbeat function is used to display a heartbeat animation on the LED matrix.
// The function has several variables that control the position of the heartbeat, the direction of the animation, and whether the animation is displaying a full or empty heart.
// The function uses a switch statement to control the different states of the animation, including the position of the heartbeat, the direction of the animation,
// and whether the animation is displaying a full or empty heart.
// Overall, the selected code is a part of a larger program that controls an LED matrix display.
// The code includes several functions that are used to display different types of animations,
// and each function has its own set of parameters and variables that control how the animation is displayed.
...
// the mode switch is pressed.
{
    ...
    case 5:
        ...
        break;
    // case 5: bRestart = graphicArrowScroll(bRestart);    break;
    // case 6: bRestart = graphicRandom(bRestart);         break;
    // case 7: bRestart = graphicArrowRotate(bRestart);    break;
    // case 8: bRestart = graphicSinewave(bRestart);       break;
    // case 9: bRestart = graphicMidline2(bRestart);       break;
    default:
        state = 0;
    }
}
